...
echo '    "interfaces": ['
first=true
ip -json addr show | jq -r '.[] | select(.addr_info != null) | .ifname + " " + (.addr_info[] | select(.family == "inet") | .local)' 2>/dev/null | while read interface ip; do
...
echo '    ],'
echo '    "connections": {'
echo '      "established": "'$(ss -tuln | grep ESTAB | wc -l)'",'
...
echo '    "running_services": "'$(systemctl list-units --type=service --state=running | wc -l)'",'
